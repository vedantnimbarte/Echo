...
export function PillApp() {
  ...
  // Audio cues used to live here, as WebAudio tones fired from an effect
  // watching this store. They are Rust's now (core/cues.rs), for the reason the
  // feature exists at all: this webview may not be on screen when the hotkey is
  // pressed, so a cue that depends on it cannot answer "did that register" —
  // which is the one question it is for.
