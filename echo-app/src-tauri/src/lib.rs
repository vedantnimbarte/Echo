...
pub fn run() {
    ...
    #[cfg(debug_assertions)]
    {
        let exporter = specta_typescript::Typescript::default()
            ...
            // Generated code is not authored code, and Echo's tsconfig is
            // strict: `noUnusedLocals` rejects tauri-specta's own scaffolding
            // (TAURI_CHANNEL, __makeEvents__), which exists to serve whichever
            // features a given app uses and is unused in one that uses few.
            //
            // Suppressing checks INSIDE this file costs nothing that matters:
            // every consumer still typechecks against the types it exports,
            // which is the whole point of generating it. The alternative was
            // relaxing noUnusedLocals for the entire project to accommodate one
            // file nobody writes.
